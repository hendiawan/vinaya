        ...
        <div id="topTabs-container-home">
            <div class="topTabs-header clearfix">
                ...
                <ul class="etabs tabs">
                    <li class="tab">
                        <a href="#tab1">
                            <span class="to-hide">
                                <i class="fa fa-folder-open"></i><br>Data Menu
                            </span>
            ...
            <div class="widget-content pad20f" style="padding-bottom:0">	
                <div class="alert alert-info" style="margin-bottom:0" role="alert">
                    ...
                    <ul style="padding-left: 20px !important;">
                        <li>Tabel dibawah memuat semua data Menu yang telah dimasukkan</li> 
                    </ul>
                </div>
            </div>
            ...
            <div class="widget-header">
                <div class="icon-grp" style="padding-top:10px;">
                    <a href="/menu" class="btn btn-blue">
...
@push('scripts')
	...
	var tableMenu=      $("#sppsb-datatable").DataTable({
                
	        processing: true,
	        serverSide: true,
	        ajax: '{{ url("/") }}/getdata-menu',
	        columns: [ 
                                  { data: 'id', name: 'id',sClass:'text-center' },
	            { data: 'nama_menu', name: 'nama_menu',sClass:'text-center'  }, 
                                  { data: 'harga', name: 'harga', sClass:'text-center',render:function(data){
                                     return    formatNumber(data);
                                  }},
	            { data: 'qty', name: 'qty',sClass:'text-center',searchable: true  },  
	            { data: 'harga_jual', name: 'harga_jual', sClass:'text-center',render:
                                   function(data){
                                       return formatNumber(data);
                                   }}, 
	            { data: 'total', name: 'total', sClass:'text-center', orderable: false, searchable: false },
	@can('admin-access')              { data: 'action', name: 'action', sClass:'text-center', orderable: false, searchable: false } @endcan
	        ...
	        aaSorting: []
                       ...
                       $(document).on('click', '.po-delete-menu', function () {
                                var id = $(this).attr('id');
                                $('#customLoad').show();
                                $('.popovers').popover('hide');
                                $.ajax({
