        ...
        <div id="topTabs-container-home">
            <div class="topTabs-header clearfix">
                ...
                <ul class="etabs tabs">
                    <li class="tab">
                        <a href="#tab1">
                            <span class="to-hide">
                                <i class="fa fa-folder-open"></i><br>Data Penjualan
                            </span>
            ...
            <div class="widget-content pad20f" style="padding-bottom:0">	
                <div class="alert alert-info" style="margin-bottom:0" role="alert">
                    ...
                    <ul style="padding-left: 20px !important;">
                        <li>Tabel dibawah memuat semua data penjualan yang telah dilakukan</li>
            ...
            <div class="widget-header">
                <div class="icon-grp" style="padding-top:10px;">
                    <a href="/penjualan" class="btn btn-blue">
                        <i class="fa fa-user-plus"></i> Tambah Penjualan
                    </a>
                </div>
            </div>
...
var tablePenjualan =     $("#sppsb-datatable").DataTable({

   processing: true,
   serverSide: true,
//    scrollX:true,
   ajax: '{{ url("/") }}/getdata-penjulan',
   columns: [ 
...
//       { data: 'ket', name: 'ket',sClass:'text-center'  },  
...
$(document).on('click', '.po-delete-penjualan', function () {
            var id = $(this).attr('id');
            $('#customLoad').show();
            $('.popovers').popover('hide');
            $.ajax({
                url: '{{ url("/") }}/delete-data-penjualan/' + id,
                type: "GET",
                cache: false,
                data: "",
                success: function (response) {
                    ...
                    if (response) {
                        $('#sppsb-datatable').before('<div class="alert alert-success">'
                                + '<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'
                                + '<i class="fa fa-info-circle"></i> SUKSES! Anda berhasil menghapus data penjualan</div>');
                        tablePenjualan.ajax.reload();
                    } else {
                        $('#sppsb-datatable').before('<div class="alert alert-danger">'
                                + '<button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>'
                                + '<i class="fa fa-info-circle"></i> GAGAL! Anda gagal menghapus data penjualan </div>');
                    }
                },
                error: function (e) {
