            ...
            <div class="topTabs-header clearfix">
                ...
                <ul class="etabs tabs">
                    <li class="tab">
                        <a href="#tab1">
                            <span class="to-hide">
                                <i class="fa fa-folder-open"></i><br>Data Nempeler
                            </span>
            ...
            <div class="widget-content pad20f" style="padding-bottom:0">	
                <div class="alert alert-info" style="margin-bottom:0" role="alert">
                    ...
                    <ul style="padding-left: 20px !important;">
                        <li>Tabel dibawah memuat semua data Nempeler  yang telah dimasukkan</li>
             ...
             <div class="widget-header">
                <div class="icon-grp" style="padding-top:10px;">
                    <a href="/karyawan-add" class="btn btn-blue">
                        <i class="fa fa-user-plus"></i> Tambah Nempeler
                    </a>
                </div>
            </div>
...
@push('scripts')
	...
	var tableKaryawan =    $("#sppsb-datatable").DataTable({
                
	        processing: true,
	        serverSide: true,
	        ajax: '{{ url("/") }}/getdata-karyawan',
	        columns: [ 
                                  { data: 'id', name: 'id',sClass:'text-center' },
	            { data: 'nama_karyawan', name: 'nama_karyawan',sClass:'text-center'  }, 
                                  { data: 'jabatan', name: 'jabatan', sClass:'text-center' },
	            { data: 'telp', name: 'telp',sClass:'text-center',searchable: true  },  
	            { data: 'alamat', name: 'alamat', sClass:'text-center' }, 
@can('admin-access')  	            { data: 'action', name: 'action', sClass:'text-center', orderable: false, searchable: false } @endcan
	        ...
	        aaSorting: []
            	 ...
            	 $(document).on('click', '.popovers', function (e) {
                       ...
                       $(document).on('click', '.po-delete-karyawan', function () {
                                var id = $(this).attr('id');
                                $('#customLoad').show();
                                $('.popovers').popover('hide');
                                $.ajax({
                                    url: '{{ url("/") }}/delete-karyawan/' + id,
                                    type: "GET",
                                    cache: false,
                                    data: "",
