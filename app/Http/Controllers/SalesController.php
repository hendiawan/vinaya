...
class SalesController extends Controller
{
    ...
    public function dashboard()
    {
            ...
            $data_penjualan     = Penjualan::with('sales')
...
//            dd($data_pembelian);
//        $data_pembelian    = pembelian::with('sales')
//                                                ->whereHas('sales',function($query){
//                                                      $query->where('karyawan_id',Auth::user()->karyawan_id); 
//                                                 }) 
//                                                ->sum('harga_pokok');
//        dd($data_pembelian);
        $paket1xMinum     = Penjualan::with('paket','sales')
                                              ->whereHas('sales',function($query){
                                                      $query->where('karyawan_id',Auth::user()->karyawan_id); 
                                                 }) 
        ...
        $paket10xMinum     = Penjualan::with('paket')
                                                 ->whereHas('sales',function($query){
                                                      $query->where('karyawan_id',Auth::user()->karyawan_id); 
                                                 }) 
                                                ->whereHas('paket',function($query){
        ...
        $paket30xMinum     = Penjualan::with('paket')
                                              ->whereHas('sales',function($query){
                                                      $query->where('karyawan_id',Auth::user()->karyawan_id); 
                                                 }) 
                                             ->whereHas('paket',function($query){
